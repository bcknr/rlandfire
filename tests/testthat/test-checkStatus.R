...
httptest2::with_mock_dir("_mock/checkStatus-nojob", {
  test_that("`.checkStatus_internal()` recognizes API errors", {
    ...
    expect_error(
      .checkStatus_internal(noJob_landfire_api, i = 1, max_time = 10),
...
httptest2::with_mock_dir("_mock/checkStatus", {
  test_that("`.checkStatus_internal()` returns expected", {
    landfire_api <- .build_landfire_api(
      job_id = "be7fab41-867e-41a5-b594-c469f118bc49",
      path = tempfile(fileext = ".zip")
    )
    return <- .checkStatus_internal(landfire_api, i = 1, max_time = 10, method = "none")
...
httptest2::with_mock_dir("_mock/cancelJob", {
  test_that("cancelJob() returns expected response", {
    ...
    # Check message
    expect_message(
      cancelJob("6f5ba39c-09f7-4f6d-97fa-543af185eb1b"),
