...
To demonstrate the current functionality of `rlandfire`, we will explore how forest canopy cover in Southern Rocky Mountain Ponderosa Pine Woodland changed after the 2020 Calwood fire near Boulder, Colorado.
...
We can use the function `rlandfire::getAOI()` to create an area of interest (AOI) vector with the correct format for `landfireAPI()`. `getAOI()` handles several steps for us. It ensures that the AOI is returned in the correct order (`xmin`, `ymin`, `xmax`, `ymax`) and converts the AOI to latitude and longitude coordinates if needed.

If required for our workflow, we could extend the AOI by providing an optional numeric vector of 1, 2, or 4 elements to the `extend` argument. In this case, we will extend the AOI by 1 km in all directions to provide additional context surrounding the burned area.
...
Finally, we will provide a path to a temporary file and submit a request to the LANDFIRE Product Services API with the `landfireAPI` function.
...
`landfireAPI` will automatically download your requested data into the folder provided in the path argument. If you didn't provide one, you can find the path to your data in the `$path` element of the `landfire_api` object returned by `landfireAPI()`.

The downloaded files are compressed `.zip` files. We need to unzip them before reading the file ending in `.tif`. Note: all additional metadata is included in this same directory.
