...
<!-- badges: start -->
![](https://img.shields.io/github/r-package/v/bcknr/rlandfire) [![R-CMD-check](https://github.com/bcknr/rlandfire/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/bcknr/rlandfire/actions/workflows/R-CMD-check.yaml)
<!-- badges: end -->
...
For this example, we are interested in canopy cover data for two years, 2019 (`200CC_19`) and 2022 (`220CC_22`), and existing vegetation type (`200EVT`). All available data products, and their abbreviated names, can be found in the [products table](https://lfps.usgs.gov/lfps/helpdocs/productstable.html) which can be opened by calling `viewProducts()`.
...
To do so, we specify that when `220EVT` is not equal (`ne`) to `7054`, the "condition," the canopy cover layers should be set equal (`st`) to `1`, the "change." The edit rule syntax is explained in more depth in the [LFPS guide](https://lfps.usgs.gov/lfps/helpdocs/LFProductsServiceUserGuide.pdf).
